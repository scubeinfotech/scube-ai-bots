...
(function() {
  ...
  <path d="M28 52 Q40 62 52 52" stroke="url(#rbg2)" stroke-width="3" fill="none" stroke-linecap="round"/>
        ...
        // Check if message is a callback/meeting request
        _isCallbackRequest: function(text) {
            if (!text) return false;
            const lower = text.toLowerCase();
            const phrases = [
                'call me', 'call back', 'callback', 'ring me', 'phone me',
                'arrange a call', 'schedule a call', 'book a call', 'meeting',
                'arrange meeting', 'schedule meeting', 'book meeting',
                'talk to someone', 'speak to someone', 'get a call',
                'contact me', 'have someone call', 'request a call'
            ];
            return phrases.some(p => lower.includes(p));
        },

        // Store pending callback message to send after lead form
        _pendingCallbackMessage: null,
        ...
        sendMessage: function() {
            ...
            // If user requests callback/meeting but we don't have their details → show lead form first
            if (this._isCallbackRequest(content) && !leadCollected) {
                this.addMessageToUI(content, 'user');
                input.value = '';
                this._pendingCallbackMessage = content;
                this._showLeadFormWithCallbackPrompt();
                return;
            }
        ...
        // Show lead form specifically for callback requests with context-aware messaging
        _showLeadFormWithCallbackPrompt: function() {
            const messagesDiv = document.getElementById('llm-messages');
            const chatInput = document.getElementById('llm-chat-input');
            const input = document.getElementById('llm-message-input');
            if (!messagesDiv) return;
            ...
            const inp = 'display:block;width:100%;border:none;border-bottom:1px solid #cbd5e1;background:transparent;padding:5px 2px;font-size:12px;margin-bottom:6px;box-sizing:border-box;outline:none;color:#1e293b;font-family:inherit;';
            card.innerHTML = `
                <p style="font-size:11px;color:#64748b;margin:0 0 8px;line-height:1.4;font-weight:500;">Happy to arrange that! To set up the call, I'll need your details:</p>
                <input id="llm-li-name"  type="text"  placeholder="Your Name"  style="${inp}">
                <input id="llm-li-email" type="email" placeholder="Email" style="${inp}">
                <input id="llm-li-phone" type="tel"   placeholder="Phone Number" style="${inp}margin-bottom:9px;">
                <div style="display:flex;align-items:center;gap:8px;">
                    <button id="llm-li-submit" style="flex:1;border:none;border-radius:16px;padding:6px 0;font-size:12px;font-weight:600;color:#fff;cursor:pointer;min-height:30px;">Arrange Call ›</button>
                    <span id="llm-li-skip" style="font-size:11px;color:#94a3b8;cursor:pointer;white-space:nowrap;text-decoration:underline;">skip</span>
                </div>
                <div id="llm-li-error" style="color:#ef4444;font-size:11px;margin-top:4px;display:none;"></div>
            `;
            ...
            // Wire up events - use callback-aware submit handler
            if (submitBtn) submitBtn.addEventListener('click', () => this._submitInlineLeadFormForCallback());
            const skipBtn = document.getElementById('llm-li-skip');
            if (skipBtn) skipBtn.addEventListener('click', () => this._skipInlineLeadFormCallback());

            // Allow Enter key to submit
            ['llm-li-name','llm-li-email','llm-li-phone'].forEach(id => {
                const el = document.getElementById(id);
                if (el) el.addEventListener('keydown', e => { if (e.key === 'Enter') this._submitInlineLeadFormForCallback(); });
            });
        },
        ...
        // Callback-aware submit: saves lead and confirms callback
        _submitInlineLeadFormForCallback: function() {
            const name  = (document.getElementById('llm-li-name')  || {}).value?.trim() || '';
            const email = (document.getElementById('llm-li-email') || {}).value?.trim() || '';
            const phone = (document.getElementById('llm-li-phone') || {}).value?.trim() || '';
            const errDiv = document.getElementById('llm-li-error');
            ...
            .then(() => {
                ...
                // Confirm the callback with the collected details
                const pendingMsg = this._pendingCallbackMessage;
                this._pendingCallbackMessage = null;

                // Add confirmation message showing we have their details
                this.addMessageToUI(
                    `Thank you, ${name}! Our team will call you back at ${phone} shortly. You can continue chatting if you have any other questions.`,
                    'assistant'
                );

                // Optionally send the callback request to the backend for logging
                if (pendingMsg) {
                    this._sendCallbackRequestToBackend(pendingMsg, { name, email, phone });
                }
            })
            .catch(() => {
                if (errDiv) { errDiv.style.display = 'block'; errDiv.textContent = 'Could not save. Please try again.'; }
                if (submitBtn) { submitBtn.disabled = false; submitBtn.textContent = 'Arrange Call ›'; }
            });
        },

        // Callback-aware skip: just clears the form and sends the callback message normally
        _skipInlineLeadFormCallback: function() {
            const card = document.getElementById('llm-inline-lead-form');
            if (card) card.remove();
            const chatInput = document.getElementById('llm-chat-input');
            const input = document.getElementById('llm-message-input');
            ...
            // Send the pending callback message to backend (will get generic response asking for details)
            const pendingMsg = this._pendingCallbackMessage;
            this._pendingCallbackMessage = null;
            if (pendingMsg) {
                this._sendMessageToBackend(pendingMsg);
            }
        },

        // Helper to send callback request to backend for logging
        _sendCallbackRequestToBackend: function(message, leadInfo) {
            const authHeader = this.config.authToken || this.config.apiKey || '';
            fetch(`${this.config.apiUrl}/api/chat/message/${this.config.tenantId}`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    content: message,
                    session_id: this.config.sessionId || null,
                    lead_context: leadInfo
                })
            }).catch(() => {}); // Silent fail - callback already confirmed locally
        },

        // Helper to send a message to backend
        _sendMessageToBackend: function(content) {
            this._showTypingIndicator();
            const authHeader = this.config.authToken || this.config.apiKey || '';
            fetch(`${this.config.apiUrl}/api/chat/message/${this.config.tenantId}`, {
                method: 'POST',
    ...
    (function autoInit() {
        var script = document.getElementById('scube-widget') || document.currentScript;
        if (!script) {
            // Fallback: find the last script tag that loaded this file
